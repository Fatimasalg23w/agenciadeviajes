...
export default async function TourDetail({ params }: { params: { id: string } }) {
  const res = await fetch(`http://localhost:3000/api/tours/${params.id}`, { cache: "no-store" });
  if (!res.ok) return notFound();
  const tour = await res.json();

  const vuelo = { precio: 3500, origen: "CDMX", destino: tour.destinos?.[0], fecha: tour.fecha_inicio };
  const hotel = { precio: 4200, nombre: tour.itinerario?.[0]?.hotel_sugerido || "Hotel incluido" };

  const total = vuelo.precio + hotel.precio + (tour.precio_base || 0);
  const totalConComision = total * 1.15;

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="relative h-[50vh] w-full">
        <img src={tour.imagenes?.[0]} alt={tour.nombre} className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/40 flex flex-col justify-center items-center text-center text-white">
          ...
          <p className="text-lg md:text-xl">{tour.destinos?.join(", ")}</p>
      ...
      <div className="max-w-5xl mx-auto p-8 space-y-10">
        ...
        <div>
          <h2 className="text-2xl font-bold text-orange-700 mb-6">
            Itinerario de {tour.itinerario?.length} días
          </h2>
          <ul className="grid gap-6 md:grid-cols-2">
            {tour.itinerario?.map((d: any) => (
              <li key={d.dia} className="bg-white border rounded-lg shadow-md p-4">
                <strong className="text-orange-600">Día {d.dia}:</strong> {d.ciudad} — {d.actividad}
                {d.hotel_sugerido && (
                  <div className="text-sm text-gray-500 mt-2">
                    Hotel sugerido: {d.hotel_sugerido}
                  </div>
                )}
              </li>
            ))}
